...
ButtonDHCP:
{
	...
	IfMsgBox Yes
	{
		GuiControlGet, AdaptersDDL
		AdaperIndex := GetAdapterIndex(AdaptersDDL)
		psArgs := "Set-NetIPInterface -InterfaceIndex" AdapterIndex " -Dhcp Enabled `; "
		;psArgs := "Set-DnsClientServerAddress -InterfaceAlias '" AdaptersDDL " -ResetServerAddresses"
		;Run PowerShell -Command %psArgs%
		Run, *RunAs PowerShell.exe -Command %psArgs% Set-DnsClientServerAddress -InterfaceIndex %AdapterIndex% -ResetServerAddresses,, Hide
