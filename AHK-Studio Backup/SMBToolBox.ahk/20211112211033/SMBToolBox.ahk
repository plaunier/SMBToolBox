...
ButtonSTATIC:
{
	...
	{
		;GuiControl,, ScriptText, Adapter= %AdaptersDDL%`nUseable= %Useable%`nSubnet= %SubnetsDDL%`nGateway= %Gateway%`nDNS1= %DNS1%`nDNS2=%DNS2%
		Run *RunAs, %comspec% /c netsh interface ipv4 set address name="%AdaptersDDL%" static %Useable% %SubnetsDDL% %Gateway%,,hide
		;run, %comspec% /c netsh interface ipv4 set address name="%AdaptersDDL%" static %Useable% %SubnetsDDL% %Gateway%,,hide
		;run, %comspec% /c netsh interface ipv4 set dns name="%AdaptersDDL%" static %DNS1%,,hide
		if (UnqDNS2 = "") 
		{    
			run, %comspec% /c netsh delete dnsserver "%AdaptersDDL%" index=2,,hide
			return
		}
		Else
			run, %comspec% /c  netsh interface ip add dns "%AdaptersDDL%" %DNS2% index=2,,hide
	}
	return
}
...
{
	GuiControlGet, AdaptersDDL
	run, %ComSpec% /c netsh interface ip set address "%AdaptersDDL%" dhcp,,hide
	return
}
