...
ButtonCreateTunnel:
{
	...
	If (ValidIP(TenDot)) {
		...
		;loginArg = -ssh %JumpBoxes[1].address% -P %JumpBoxes[1].port% -l %JumpBoxes[1].user% -pw %JumpBoxes[1].pw%;
		loginArg := "-ssh " JumpBoxes[1].address
		MsgBox % loginArg
...
OnLoad() {
	...
	JumpBoxes := []
	JumpBoxes[1] := {"name": "NorthEast 1", "address": "24.24.43.132", "port": "22", "user": "bctechcpe", "pw": "T3chBCcp3"}
    JumpBoxes[2] := {"name": "NorthEast 2", "address": "24.24.43.133", "port": "22", "user": "bctechcpe", "pw": "T3chBCcp3"}
    JumpBoxes[3] := {"name": "Test Box", "address": "192.168.1.202", "port": "22", "user": "technician", "pw": "technician"}
