...
ButtonSTATIC:
{
	...
	If !(ValidIP(Gateway))
	{
		...
		IfMsgBox Yes
		{
			...
			Run, *RunAS PowerShell.exe -NoExit -Command 
			$adapter = Get-NetAdapter | ? {$_.Status -eq \""up\""} `; If (($adapter | Get-NetIPConfiguration).IPv4Address.IPAddress) { $adapter | Remove-NetIPAddress -AddressFamily \""IPv4\"" -Confirm:$false } `; If (($adapter | Get-NetIPConfiguration).Ipv4DefaultGateway) { $adapter | Remove-NetRoute -AddressFamily \""IPv4\"" -Confirm:$false } `; $adapter | New-NetIPAddress -AddressFamily \""IPv4\"" -IPAddress \""%Useable%\""
