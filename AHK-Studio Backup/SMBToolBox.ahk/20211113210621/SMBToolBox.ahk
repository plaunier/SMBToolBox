...
ButtonSTATIC:
{
	...
	If (ValidIP(Gateway) && ValidIP(Useable))
	{
		...
		IfMsgBox Yes
		{
			...
			AdapterIndex := 10
			psArgs := "New-NetIPAddress -InterfaceIndex " AdapterIndex " -IPAddress " Useable " -PrefixLength 24 -DefaultGateway " Gateway " `; Set-DnsClientServerAddress -InterfaceIndex " AdapterIndex " -ServerAddresses """ DNS1 " , " DNS2 """
			;Run, *RunAS PowerShell.exe -ExecutionPolicy Bypass -Command %psArgs%
			Run, *RunAS PowerShell.exe -Command %psArgs%
			Tms := 20
			Loop, %Tms% 
			{
				Position := 100/Tms * A_Index
