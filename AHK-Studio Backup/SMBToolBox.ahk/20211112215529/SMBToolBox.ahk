...
ButtonSTATIC:
{
	...
	GuiControlGet, DNS1
	GuiControlGet, DNS2
	;args1 := " /c netsh interface ip set address " %AdaptersDDL% " static " %Useable% " " %SubnetsDDL% " " %GateWay%
	args1 := " /c netsh interface ip set address " %AdaptersDDL% " static "
	MsgBox % args1
	return
	If (ValidIP(Gateway) && ValidIP(Useable))
	{
		...
		IfMsgBox Yes
		{
			...
			args1 := " /c netsh interface ip set address " %AdaptersDDL% " static "
			MsgBox % args1
			;agrs2 := " & netsh interface ip set dns " %AdaptersDDL% " static " %DNS1%
			;args3 := " & netsh interface ip add dns " %AdaptersDDL% " addr=" %DNS2% " index=2"
			;args := %args1% %args2% %args3%
			
			;Run, *RunAs %comspec% %args1%
		}
	}
	return
}
