...
ButtonSTATIC:
{
	...
	If (ValidIP(Gateway) && ValidIP(Useable))
	{
		...
		IfMsgBox Yes
		{
			...
			;psArgs := "New-NetIPAddress –IPAddress " Useable " -DefaultGateway " Gateway " -PrefixLength " CIDR " -InterfaceIndex (Get-NetAdapter).InterfaceIndex"
			CIDR := 24
			psArgs := "Set-NetIPAddress -InterfaceIndex " AdapterIndex " -IPAddress " Useable " -PrefixLength " CIDR " -DefaultGateway " Gateway " `; Set-DnsClientServerAddress -InterfaceIndex " AdapterIndex " -ServerAddresses """ DNS1 " , " DNS2 """"
			;Run, *RunAS PowerShell.exe -ExecutionPolicy Bypass -Command %psArgs%
...
{
	;GuiControlGet, AdaptersDDL
	;MsgBox % GetIPByAdaptor(AdaptersDDL)
	;Return
	...
	IfMsgBox Yes
	{
		...
		GuiControl,, ScriptText, Set-NetIPInterface -InterfaceAlias %AdaptersDDL% -Dhcp Enabled `; Set-DnsClientServerAddress -InterfaceIndex %AdapterIndex% -ResetServerAddresses
		Return
		Run, *RunAs PowerShell.exe -Command Set-DnsClientServerAddress -InterfaceIndex %AdapterIndex% -ResetServerAddresses `; Set-NetIPInterface -InterfaceAlias %AdaptersDDL% -Dhcp Enabled
