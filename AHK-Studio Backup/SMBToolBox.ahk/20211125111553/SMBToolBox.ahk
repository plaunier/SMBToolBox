...
ButtonSTATIC:
{
	...
	If !(ValidIP(Gateway))
	{
		...
		IfMsgBox Yes
		{
			...
			Run, *RunAS PowerShell.exe -NoExit -Command $adapter = Get-NetAdapter | ? {$_.Status -eq \""up\""} `; $adapter | New-NetIPAddress -AddressFamily \""IPv4\"" -IPAddress \""%Useable%\"" -PrefixLength %CIDR% -DefaultGateway \""%Gateway%\""
