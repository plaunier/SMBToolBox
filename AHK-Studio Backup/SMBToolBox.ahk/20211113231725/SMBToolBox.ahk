...
AdaptersDDL: 
{
	...
	AdapterIndex := GetAddressIndex(AdaptersDDL)
	MsgBox % AdaptorIndex
	Return
}
	...
	ButtonDHCP:
	{
		...
		IfMsgBox Yes
		{
			...
			Run, *RunAs PowerShell.exe -Command Set-DnsClientServerAddress -InterfaceIndex %AdapterIndex% -ResetServerAddresses
	...
	GuiCreate() {
		...
		;AdapterIndex := getAdapterIndex(getAddressIndex)
		;MsgBox
		Return
	}
	...
	GetAdapterIndex(a){
		; Get-NetAdapter -Name "Ethernet" | Format-List -Property IfIndex
		RunWait, PowerShell.exe Get-NetAdapter -Name %AdaptersDDL% | Format-List -Property IfIndex | Out-File -FilePath %A_Temp%\NetInfo.txt -Width 300,, Hide
		Loop, read, %A_Temp%\NetInfo.txt
		{
			strPos := InStr(A_LoopReadLine, ":")
			If (strPos != 0) {
				StringRight, i, A_LoopReadLine, StrLen(A_LoopReadLine)-strPos
				i := Trim(AdaptersIndex)
				Break
			}	
		}
